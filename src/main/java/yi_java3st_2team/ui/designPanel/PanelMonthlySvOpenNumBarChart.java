...
public class PanelMonthlySvOpenNumBarChart extends JFXPanel implements InitScene{
	public PanelMonthlySvOpenNumBarChart() {
	}
	...
	private ObservableList<XYChart.Series<String, Number>> getChartData() {
		...
		List<String> listSv = new ArrayList<>();//적금건수
		List<String> listZero = null;
		
		try {
			listSv = service.showOpenSvMonth();
			listZero = new ArrayList<>(listSv.size());
		...
		for(int i=1; i<13; i++) {
			for(int j=0; j<listSv.size(); j++) {
				if(("0"+Integer.toString(i)).equals(listSv.get(j)) && j==(listSv.size()-1)) {
						list.add(getChartData(i+"월", Integer.toString(listSv.size())));
				}else {
					listZero.add("0");
