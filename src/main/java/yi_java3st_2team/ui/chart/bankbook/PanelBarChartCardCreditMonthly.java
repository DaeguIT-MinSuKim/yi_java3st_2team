...
public class PanelBarChartCardCreditMonthly extends JFXPanel implements InitScene{
	private BarChart<String, Number> barChart;
	private CardService service;
	public PanelBarChartCardCreditMonthly() {
	...
	public Scene createScene() {
		...
		barChart.setTitle("고객별 신용카드 월간 거래 내역 조회");
	...
	private ObservableList<XYChart.Series<String, Number>> getChartData() {
		...
		try {
			list1 = service.cardInfoMonthly("김가나");
			list2 = service.cardInfoMonthly("김다라");
			list3 = service.cardInfoMonthly("김마바");
			list4 = service.cardInfoMonthly("김사아");
			list5 = service.cardInfoMonthly("김자차");
			if(list1.size()==0) {
				accountInfo1 = new AccountInfo("김가나", 0);
			}
			else {
				for(CardInfo info : list1) {
					if(info.getDiv().equals("신용카드")) {
