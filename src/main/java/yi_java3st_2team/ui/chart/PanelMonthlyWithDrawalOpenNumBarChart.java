...
public class PanelMonthlyWithDrawalOpenNumBarChart extends JFXPanel implements InitScene{
	public PanelMonthlyWithDrawalOpenNumBarChart() {
	}
	...
	private ObservableList<XYChart.Series<String, Number>> getChartData() {
		...
		List<String> listWd = new ArrayList<>();//출금건수
		List<String> listZero = null;
		
		try {
			listWd = service.showWithdrawalMonth();
			listZero = new ArrayList<>(listWd.size());
		...
		for(int i=1; i<13; i++) {
			for(int j=0; j<listWd.size(); j++) {
				if(("0"+Integer.toString(i)).equals(listWd.get(j)) && j==(listWd.size()-1)) {
						list.add(getChartData(i+"월", Integer.toString(listWd.size())));
				}else {
					listZero.add("0");
