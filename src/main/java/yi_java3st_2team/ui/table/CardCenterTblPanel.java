...
public class CardCenterTblPanel extends AbsCenterTblPanel<Card> {
	...
	protected void setTblWidthAlign() {
		...
		setColumnWidth(150,50,100,50,100,150,100);
	}

	@Override
	protected Object[] toArray(Card item) {
		chkCard = item;
		return new Object[] {item.getCardNum(),item.getCustCode().getCustName(),item.getPlanCode().getPlanName(),item.getCardNum().substring(7).equals("0")?"체크카드":"신용카드",item.getCardSecuCode(),new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getCardIssueDate()),item.getCardNum().substring(8).equals("1")?item.getCardBalance():item.getCardLimit()};
	}
	...
	protected void updateRow(Card item, int updateIdx) {
		...
		model.setValueAt(item.getCardNum().substring(8).equals("1")?"체크카드":"신용카드", updateIdx, 3);
		...
		model.setValueAt(item.getCardLimit(), updateIdx, 6);
	}
	...
	protected Card getSelectedItem() {
		...
		if(cardDiv.equals("체크카드")) {
			...
			card.setCardLimit((int)model.getValueAt(selIdx, 6));
		}
		return card;
	}
	...
	protected String[] getColumns() {
		return new String[] {"카드번호","고객이름","상품이름","카드구분","카드보안코드","카드발급일",chkCard.getCardNum().substring(8).equals("1")?"카드잔액":"카드한도"};
	}
